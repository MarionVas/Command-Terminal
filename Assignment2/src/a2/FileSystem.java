...
public class FileSystem {
  ...
  public Object getObject(String name){
    ...
    if (name.startsWith("/")){
      ...
      for (int i = 0; i < Manager.size(); i++){
        if (result == null){
          result = getObjRecurs(name, "/" + ((Folder) Manager.get(i)).getName(),Manager.get(i));
        }
      }
  ...
  public Object getObjRecurs(String name, String currName,Folder dirrOrFile){
    ...
    Object result = null;
    for (int i = 0; result == null && allChildren != null &&i < allChildren.size(); i++){
      if (allChildren.get(i).getClass().equals(Folder.class)){
        result = getObjRecurs(name, currName + "/" + ((Folder) allChildren.get(i)).getName(), (Folder)allChildren.get(i));
      }
      else{
